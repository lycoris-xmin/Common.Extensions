...
namespace Lycoris.Common.Helper
{
    ...
    {
        private static Dictionary<string, ConfigUser> ConfigUserMap = new Dictionary<string, ConfigUser>();
        /// <summary>
        ...
        /// <param name="path">文件路径，不包含文件名，文件名默认使用：config.ini</param>
        /// <returns></returns>
        public static ConfigUser UseConfigPath(string path)
        {
            if (ConfigUserMap.TryGetValue(path, out ConfigUser? val) && val != null)
                return val;

            ConfigUserMap[path] = new ConfigUser(path);

            return ConfigUserMap[path];
        }
        ...
        /// <param name="fileName">配置文件名</param>
        /// <returns></returns>
        public static ConfigUser UseConfigPath(string path, string fileName)
        {
            if (!Directory.Exists(path))
                FileHelper.EnsurePathExists(path);
    ...
    public class ConfigUser
    {
        ...
        public ConfigUser(string configPath)
        {
        ...
        /// <param name="section">节名称，默认为 "config"。</param>
        /// <returns>配置值字符串。</returns>
        public string GetValue(string key, string defaultValue = "0", string section = "config") => Ini.Get(section, key, defaultValue);
        ...
        /// <param name="section">节名称，默认为 "config"。</param>
        /// <returns>配置值字符串。</returns>
        public T? GetValue<T>(string key, string defaultValue = "0", string section = "config")
            => GetValue(key, defaultValue, section).ToTryObject<T>();
        ...
        /// <param name="section">节名称，默认为 "config"。</param>
        /// <returns>整数值。</returns>
        public int GetInt(string key, int defaultValue = 0, string section = "config") => Ini.GetInt(section, key, defaultValue);
        ...
        /// <param name="section">节名称，默认为 "config"。</param>
        /// <returns>布尔值。</returns>
        public bool GetBool(string key, bool defaultValue = false, string section = "config") => Ini.GetBool(section, key, defaultValue);
        ...
        /// <param name="section">节名称，默认为 "config"。</param>
        /// <param name="comment">可选注释，保存时会添加在该行尾部。</param>
        public void SetValue(string key, int value, string section = "config", params string[] comment)
        {
            Ini.Set(section, key, value, comment.ToList() ?? new List<string>());
            Ini.Save(ConfigPath);
        }
        ...
        /// <param name="section">节名称，默认为 "config"。</param>
        /// <param name="comment">可选注释，保存时会添加在该行尾部。</param>
        public void SetValue(string key, bool value, string section = "config", params string[] comment)
        {
            Ini.Set(section, key, value, comment.ToList() ?? new List<string>());
            Ini.Save(ConfigPath);
        }
        ...
        /// <param name="section">节名称，默认为 "config"。</param>
        /// <param name="comment">可选注释，保存时会添加在该行尾部。</param>
        public void SetValue(string key, string value, string section = "config", params string[] comment)
        {
            Ini.Set(section, key, value, comment.ToList() ?? new List<string>());
            Ini.Save(ConfigPath);
        }
        ...
        /// <param name="section">节名称，默认为 "config"。</param>
        /// <param name="comment">可选注释，保存时会添加在该行尾部。</param>
        public void SetValue<T>(string key, T value, string section = "config", params string[] comment) where T : class, new()
        {
            Ini.Set(section, key, value.ToJson(), comment.ToList() ?? new List<string>());
            Ini.Save(ConfigPath);
        }
        ...
        /// 重新加载配置文件并清除缓存。
        /// </summary>
        public void Reload() => _ini = IniFile.Load(ConfigPath);
    }
}
